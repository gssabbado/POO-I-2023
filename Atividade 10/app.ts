...
export class App {
    ...
    // Verifica se o usuario possui ainda um aluguel aberto antes de remover
    async removeUser(email: string): Promise<void> {
        await this.findUser(email)
        await this.userRepo.remove(email)

        const openRents = await this.rentRepo.findOpenRent(email);
        
        if (openRents.length > 0) {
            throw new Error(`user has open rent.`);
        }
